...
在提供方行的上方，页面显示 DeepSeek 账户余额：每次挂载读取一次，只由其明确的重试按钮刷新——余额不是实时数据，而本页其余部分本来就靠推送失效收敛而非轮询。读取失败会渲染成失败，绝不渲染成余额 0：这是两件不同的事，只有其中一件意味着账户需要充值。MiniMax 没有余额行，因为它未公开这样的端点；该行改为指向它的控制台。
